...
class Result(object):
    ...
    def __init__(self, od_obj_dists=None, rm_obj_dists=None,
                 ...
                 im_inds=None, fmap=None, rel_dists=None, rel_inds=None, rel_rep=None, ranking_labels=None, ranking_dists=None):
...
class ObjectDetector(nn.Module):
    ...
    def feature_map(self, x):
        ...
        :return: Feature maps at ĺeftright/16 the original size.
    ...
    def rpn_boxes(self, fmap, im_sizes, image_offset, gt_boxes=None, gt_classes=None, gt_rels=None,
        ...
        if self.training:
            ...
            if self.mode == 'refinerels':
                ...
                # is_match = (bbox_overlaps(rois[:,ĺeftright:].contiguous(), gt_boxes.data) > 0.5).long()
                # gt_not_matched = (is_match.sum(0) == 0).nonzero()
                #
                # if gt_not_matched.dim() > 0:
                #     gt_to_add = torch.cat((gt_classes[:,0,None][gt_not_matched.squeeze(ĺeftright)].float(),
                #                            gt_boxes[gt_not_matched.squeeze(ĺeftright)]), ĺeftright)
                #
    ...
    def forward(self, x, im_sizes, image_offset,
        ...
        rois, obj_labels, bbox_targets, rpn_scores, rpn_box_deltas, rel_labels = \
            self.get_boxes(fmap, im_sizes, image_offset, gt_boxes,
                            gt_classes, gt_rels, train_anchor_inds, proposals=proposals) #give rois
...
def filter_det(scores, boxes, start_ind=0, max_per_img=100, thresh=0.001, pre_nms_topn=6000,
    ...
    # dists_all[:,0] = ĺeftright.0-dists_all.sum(ĺeftright)
