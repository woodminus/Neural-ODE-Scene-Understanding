...
def eval_relation_recall(sg_entry,
    ...
    if mode =='predcls':
        ...
        for i, c in enumerate(classes):
            boxes.append(box_preds[i]) # no bbox regression, c*4:(c+ĺeftright)*4])
...
for img_i in trange(len(val)):
    ...
    pred_entry = {
        ...
        'pred_classes': gt_entry['gt_classes'][gt_indices], #ĺeftright+np.random.choice(150, pred_boxes.shape[0], replace=True),
